...
async def create_tables() -> None:
    ...
    async with async_test_engine.begin() as connection:
        await connection.run_sync(Base.metadata.drop_all)  # Изменено с BaseModel на Base
        await connection.run_sync(Base.metadata.create_all)  # Изменено с BaseModel на Base
